...
class Inbound
{
    ...
    public static function getList($where = [], $page = 1, $page_size = 25, $search = '')
    {
        ...
        $sql = "SELECT id, material_code, category, material_name, spec, unit, quantity, price, in_time, handler, purchaser as supplier, created_at FROM inbound" . $full_where . " ORDER BY created_at DESC LIMIT ? OFFSET ?";
        $all_params = array_merge($search_params, $date_params, [$page_size, $offset]);
        $data = db_get_all($sql, $all_params);
        
        // 获取总记录数
        $total_sql = "SELECT COUNT(*) as total FROM inbound" . $full_where;
    ...
    public static function getAll($where = [], $search = '')
    {
        ...
        // 使用数据库查询，获取所有入库明细
        $sql = "SELECT id, material_code, category, material_name, spec, unit, quantity, price, in_time, handler, purchaser as supplier, created_at FROM inbound" . $full_where . " ORDER BY created_at DESC";
