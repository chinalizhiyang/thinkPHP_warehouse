...
class Inbound
{
    ...
    public function index()
    {
        ...
        // 获取入库单列表
        $result = InboundModel::getList($where, $page, $page_size, $search);
        ...
        // 渲染入库单列表内容
        $content = $this->renderInboundListContent($inbounds, $result['total'], $page, $page_size, $search, $start_date, $end_date);
    ...
    // 渲染入库单列表内容
    private function renderInboundListContent($inbounds, $total, $page, $page_size, $search = '', $start_date = '', $end_date = '')
    {
        ob_start();
        ?>
        <div class="card">
            <div class="card-header">
                ...
                <form class="form-inline" method="get" action="/inbound" style="display: flex; flex-wrap: nowrap; align-items: center; gap: 8px;">
                    ...
                    <a href="/inbound/export-csv?search=<?php echo urlencode($search); ?>&start_date=<?php echo urlencode($start_date); ?>&end_date=<?php echo urlencode($end_date); ?>" class="btn btn-success btn-sm" style="white-space: nowrap;"><i class="fa fa-download"></i> 导出CSV</a>
                    <a href="/inbound/add" class="btn btn-primary btn-sm" style="white-space: nowrap;"><i class="fa fa-plus"></i> 创建入库单</a>
                </form>
            </div>
            <div class="card-body">
    ...
    public function getMaterials()
    {
        ...
        // 如果有搜索关键词，进行过滤
        if (!empty($search)) {
            $materials = array_filter($materials, function($material) use ($search) {
                return stripos($material['code'], $search) !== false || 
                       stripos($material['name'], $search) !== false ||
                       stripos($material['category'], $search) !== false;
            });
        }
    ...
    public function exportCsv()
    {
        ...
        // 获取所有数据（不分页）
        $result = InboundModel::getAll($where, $search);
        $inbounds = $result['data'];
        
        // 设置 CSV文件头部
        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename="入库单_' . date('Y-m-d_H-i-s') . '.csv"');
        ...
        // 写入表头
        fputcsv($output, ['ID', '物料编码', '类别', '物料名称', '规格', '单位', '数量', '单价', '总价', '入库日期', '采购人', '备注']);
